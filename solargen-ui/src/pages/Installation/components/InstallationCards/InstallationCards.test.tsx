...
describe('InstallationCards', () => {
    it('renders skeletons when data is undefined', () => {
        const {container} = render(<InstallationCards data={undefined} />);
    ...
    it('displays the total capacity when data is loaded', () => {
        render(<InstallationCards data={mockInstallation} />);
    ...
    it('identifies the site with the highest capacity factor as the best performing site', () => {
        ...
        const bestSiteCard =
            screen.getByText('Site le plus performant').closest('.card') ??
            screen.getByText('Site le plus performant').parentElement?.parentElement;
